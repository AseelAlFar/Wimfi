...
export const getAllMemories = async (): Promise<Record<string, string>> => {
  if (!userId) return {};
  const querySnap = await getDocs(collection(db, "users", userId, "memories"));
  const result: Record<string, string> = {};
  querySnap.forEach((doc) => { result[doc.id] = doc.data().location; });
  return result;
};
